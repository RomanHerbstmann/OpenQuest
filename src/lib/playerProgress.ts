...
export function restoreProgress(value: Partial<PlayerProgress>, legacy: boolean): PlayerProgress {
  ...
  const events = !legacy && Array.isArray(value.scanEvents) ? value.scanEvents.filter((event): event is ScanEvent =>
    Boolean(event && typeof event.id === 'string' && (event.treeId === null || typeof event.treeId === 'string') && typeof event.species === 'string' && typeof event.scannedAt === 'string' && Number.isFinite(Date.parse(event.scannedAt)) && (event.assetId === null || typeof event.assetId === 'string')),
  ) : [];
...
export function withScan(current: PlayerProgress, event: ScanEvent, xpReward = 0): PlayerProgress {
  ...
  return {
    ...current, xp, level: levelForXp(xp),
    discoveredTrees: !event.treeId || current.discoveredTrees.includes(event.treeId) ? current.discoveredTrees : [...current.discoveredTrees, event.treeId],
