...
- Shared player context with local progress and level calculation (`src/lib/playerProgress.ts`). V3 seeds the Eiche, Linde and Zierkirsche cards as revealed; other card images use the supplied OpenQuest back until a new scan unlocks them. The v1 demo save is migrated without treating earlier Festtanne tests as a stage reveal. Signed in, the header shows the server points instead of the local XP.
- Camera scan in the collection and tree details: when starting from the Baumbuch, optionally choose a specific tree point (or scan freely), capture or choose a photo, see a scan animation and save each scan separately in browser `localStorage`. Repeated scans of the same individual tree increase the find count and history while its card image unlocks only once. A mobile camera-picker fallback is available if the live preview fails. Photos are checked by the real tree photo verification (see below); the presentation Festtanne keeps its scripted demo answer. Without verification (no API key) the dialog falls back to an explicitly labeled demo species response that the player can correct and that is queued as a pending species suggestion for the admin demo.
...
- Admin review demo at `/admin` with eight sample reports plus local scan suggestions, search, status filters, map context, review notes, local decisions and CSV/GeoJSON export. Scan suggestions retain the selected tree ID and remain pending until reviewed.
...
- [ADR-0012: Badges](docs/adr/0012-badges.md)
- [ADR-0013: Sync on request and snapshots in S3](docs/adr/0013-sync-on-request-and-snapshots-in-s3.md)
