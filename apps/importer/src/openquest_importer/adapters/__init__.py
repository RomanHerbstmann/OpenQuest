...
from openquest_importer.adapters.base import DataSourceAdapter
...
def available_adapters() -> dict[str, type[DataSourceAdapter]]:
    return {ep.name: ep.load() for ep in entry_points(group=ENTRY_POINT_GROUP)}


def create_adapter(key: str, options: Mapping[str, Any] | None = None) -> DataSourceAdapter:
    ...
    if not issubclass(adapter_cls, DataSourceAdapter):
        raise UnknownAdapterError(f"Entry point '{key}' is not a DataSourceAdapter")
