...
- **Import** is done only by the importer (`apps/importer`, [README](apps/importer/README.md)), **not by the .NET API**; the API only reads assets, data sources and sync runs. An admin can ask for a sync (`POST /admin/sync`): the API records it in `sync_request` and the importer (`serve`) runs it ([ADR-0013](docs/adr/0013-sync-on-request-and-snapshots-in-s3.md)). Raw downloads can be kept in S3 (MinIO) so that the API can serve them (`GET /admin/sync/runs/{id}/snapshot`). Assets are synced into our own database on a schedule, so the city's platform is not a runtime dependency of the game.
...
- **Badges:** earned by what a player did (approvals, task types, points, cards, genera, rarity, new trees); criteria are JSON in `badge.criteria`, the rules are pure (`BadgeRules` in the core), evaluated after every approval and when an admin adds or changes a badge; one award per player and badge by primary key ([ADR-0012](docs/adr/0012-badges.md)).
