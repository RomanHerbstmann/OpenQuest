...
Packages so far: `packages/tree-verification` (photo verification, framework free), `packages/adapters/de-muenster` (Münster tree WFS as `NearbyTreeProvider`, TypeScript). The C# core lives next to them in `packages/core/OpenQuest.Core`.
