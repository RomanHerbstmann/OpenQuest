...
erDiagram
    ...
    BADGE {
        uuid id PK
        varchar key UK
        varchar name "i18n key for default badges"
        varchar description
        varchar icon
        jsonb criteria "{type, count, rarity?, taskType?}"
        int reward_points "bonus, 0 = none"
        boolean is_active
        timestamptz created_at
        timestamptz updated_at
    }
...
- **Badges ([ADR-0012](../adr/0012-badges.md)):** `badge` has `description`, `reward_points` (bonus booked in the ledger with `point_transaction.reason = badge_reward`), `is_active` and timestamps in addition to the diagram; `criteria` is `{"type": "approved_submissions|task_type|points|cards|distinct_genera|rarity_cards|new_trees", "count": n, "rarity"?, "taskType"?}`. `user_badge` (primary key `user_id` + `badge_id`) makes an award happen once. The ten default badges are inserted at start-up if missing; an admin manages the rest.
- **Sync requests ([ADR-0013](../adr/0013-sync-on-request-and-snapshots-in-s3.md)):** `sync_request` (`data_source_key` = a source key or `*` for all enabled sources, `force`, `accept_schema_change`, `requested_by`, `requested_at`, `started_at`, `finished_at`, `status` = `pending | running | succeeded | failed`, `sync_run_id`, `error`) is written by the API and answered by the importer; a partial unique index allows one pending or running request per source key.
- **Not implemented yet:** `MEDIA.captured_at` stays null (the EXIF time is dropped with the rest of the metadata).
- **Import is not part of the API.** Assets, data sources and sync runs are written by the separate importer; the API only reads them (and reads `sync_run` / `asset_snapshot` for `GET /admin/sync/runs` and `GET /admin/assets/{id}/history`). The one thing the API writes towards the importer is `sync_request`: an admin's wish to sync now. How `external_id` is assigned (or left empty for sources without ids) and how assets are matched across syncs is decided by the importer.
...
- **Snapshots and history:** written by the importer as described in "Snapshots and history". The API shows them read-only: `GET /admin/sync/runs` lists the runs (status, counters, error), `GET /admin/assets/{id}/history` the versions of an asset (`created`, `updated`, `removed`), `GET /admin/sync/runs/{id}/snapshot` the raw download of a run when the importer keeps its snapshots in S3.
