...
- **`asset.attributes` for trees** additionally contain `genus_raw` (value as delivered). `quality_flags` uses exactly the enum above: `placeholder_genus` (`Baum Amt62`, `Baumgruppe`, `Standort`, `Leerer*`, `Unbekannt`, empty), `typo_corrected` (a known typo such as `Catalpha` was fixed, or a `-Hybride` suffix was stripped; the source value stays in `genus_raw`), `near_duplicate` (another tree < 1 m away). Missing or short street keys get no flag: the key is `null` respectively zero-padded. Quests can select assets with a JSONB containment filter on these, e.g. `{"genus": null}`. The `condition` enum is `good | damaged | dead | gone`, used by the `condition_report` task.
...
- **Snapshots and history** are implemented as described in "Snapshots and history":
  - Every sync run stores the download exactly as delivered (`snapshot_key` = `snapshots/{data source}/{run id}.geojson` in the blob store; reload via `GET /admin/sync/runs/{id}/snapshot`), the number of records (`record_count`) and `schema_hash` (SHA-256 over the sorted list of field names the source delivered).
  - If `schema_hash` differs from the last successful run, the run fails (the download is kept anyway). It is continued deliberately with `POST /admin/sync?acceptSchemaChange=true`.
  - `asset_snapshot` gets a row only for `created`, `updated` (source hash differs, also when a tree moved slightly and was re-matched) and `removed` (the last known version). An unchanged import adds nothing. `GET /admin/assets/{id}/history` lists the versions. Assets that existed before this table was introduced have no `created` row.
